...
const App: React.FC = () => {
  ...
  const handleStartTimer = () => {
    setError('');
    if (!userName.trim() || !clientName.trim()) {
      setError('Please enter your name and a client name to start.');
      return;
    }
  ...
  const handleStopTimer = useCallback(async () => {
    if (!startTime || !userName || !clientName) return;
  ...
  }, [startTime, userName, clientName]);

  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-4 transition-all duration-500">
          ...
          {!isTimerRunning ? (
            <div className="space-y-4 animate-fade-in">
              <input
                ...
                className="w-full bg-slate-700 border-2 border-slate-600 rounded-lg px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition"
              />
              <input
                type="text"
                value={clientName}
